...
def lambda_handler(event, context):
    ...
    from sklearn.metrics import mean_absolute_error, \
        mean_squared_error  # Packages for measuring model performance / errors
    ...
    n_scores = cross_val_score(model, X_trainl, y_trainl, scoring='neg_mean_absolute_error', cv=cv, n_jobs=-1,
                               error_score='raise')
    print('RepeatedKFold data validation MAE: %.3f (%.3f)' % (mean(n_scores), std(n_scores)))
    ...
    print("Mean Squared error test Data Accuracy:", metrics.mean_squared_error(test_targets, y_pred))

    from sklearn.model_selection import cross_val_score

    lm = linear_model.LinearRegression()
    scores = cross_val_score(lm, X_testl, y_testl, scoring='neg_mean_squared_error', cv=10)
    print('Linear regresssion Cross validation data MAE: %.3f (%.3f)' % (mean(scores), std(scores)))
    # scores
    ...
    print('Root Square Mean Square Error (RSME): ' + str(
        np.round(mean_squared_error(y_test_unscaled, y_pred_unscaled, squared=False), 2)))
